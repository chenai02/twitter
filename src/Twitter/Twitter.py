...
class Twitter:
    ...
    def parse_message(self, soup, currentTime):
        # 获取日期
        logger.info(soup.find("time")["datetime"])
    ...
    def crash_new(self, twitter_user, currentTime):
        ...
        try:
            ...
            logger.error("No such window: target window already closed.")
            return '', '-2', [], uuid
    ...
    def download_image(url, save_path):
        try:
            ...
            logger.error(f"Error downloading the image from {url}: {e}")
        except IOError as e:
